...
{
    public class XrefFontTtfSubset: XrefFontTtfBase
	{
		...
		private readonly XrefDescendantFont m_descendantFont;
		private XrefFontContent m_content;
        private readonly XrefFontCmap m_CMAP;

        public XrefFontTtfSubset(string ttfFileName, bool useBase64) : base(ttfFileName)
		{  		
            ...
            m_descendantFont = new XrefDescendantFont(this, m_descriptor) {
                parent = this
            };
            this.sons.Add(m_descendantFont);

            m_content = new XrefFontContent (useBase64) {
                parent = this
            };
            this.sons.Add(m_content);

            m_CMAP = new XrefFontCmap {
                parent = this
            };
            this.sons.Add(m_CMAP);
		...
		public override byte[] Write() {	
			...
			return GetBytes("<</Encoding /Identity-H/ToUnicode " + m_CMAP.ID + " 0 R/Subtype /Type0/Type /Font/DescendantFonts [" + m_descendantFont.ID + " 0 R]/BaseFont /" + sixHash + "+" + fontName + ">>");
		}
        ...
        private byte[] SetGlyph(out List<int> glyphOffset)
        {
            ...
            {
                glyphSize += Get4bytePadding(glyphChar.bytes.Length);
            }
        ...
        private byte[] SetCmap() {
            ...
            for (int i = 0; i < 256; i++) {
                if (dctCharToGlyphOldNew.ContainsKey(i)) {
                    // 20 Subtable format
                    pos = SetUInt8(cmap, pos, dctCharToGlyphOldNew[i].newGlyphId);  
                } else {
