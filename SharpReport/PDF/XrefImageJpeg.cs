...
{
    public class XrefImageJpeg : IXrefImage
    {
        ...
        public XrefImageJpeg(byte[] image) {
            m_image = image;
        }

        /// <summary>
        /// Read de jpeg to extract information
        /// </summary>
        private void Analyze() {
            ...
            if (filePosition > m_image.Length - 10)
                throw new ImageException("Cannot find jpeg size") ;

            if (m_image[filePosition] != 0xFF)
                throw new ImageException("Bad JPEG image") ;
